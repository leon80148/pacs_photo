...
也支援**拍健保卡自動辨識身分證**（PP-OCRv6）與**掃描條碼**快速填入患者 ID。
...
1. 填寫「**患者 ID**」（必填，例如 `A123456789`）
   - 也可以點「**AI 辨識健保卡**」拍健保卡自動填入身分證，或點輸入框旁的**條碼圖示**掃描條碼
2. 其他患者資訊可填可不填（姓名、生日、性別）
3. 點「**拍攝**」開相機拍照，或點「**從相簿選擇**」，可多選
4. 確認照片沒問題後，按「**PACS 送信**」
5. 顯示「送出成功」就完成了

> 健保卡 OCR 與條碼掃描只用來填入患者 ID，**不會**把健保卡當成影像上傳。OCR 辨識若身分證檢查碼未通過會跳出提示，請務必人工核對。
>
> Docker 映像已內建 OCR（PP-OCRv6 模型）。若用 Python 本機執行需要 OCR，請改裝 `pip install -e .[ocr]`（見 [進階文件](docs/guide.md)）。
